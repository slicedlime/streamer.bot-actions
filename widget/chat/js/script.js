...
function processYTEmojis(list) {
    list.forEach(emoji => {
        if (Object.hasOwn(emoji, "shortcuts")) {
            emoji.shortcuts.forEach(shortcut => {
                if (shortcut.startsWith(":") && shortcut.endsWith(":")) {
                    const url = emoji.image.thumbnails.at(-1).url;
                    console.log(`Emoji ${shortcut} at ${url}`);
                    ytEmotes.set(shortcut, url)
                } else {
                    console.log(`Irregular YT emoji shortcut: ${shortcut}`);
                }
            })
        }
    });
...
function loadGlobalYTEmojiPage(pageNo) {
    // Ideally, `https://www.gstatic.com/youtube/img/emojis/emojis-svg-${pageNo}.json`, but CORS
    // Note, if re-fetching: 2 & 3 don't exist, so files need to be shifted
    // Although they all seem to contain about the same data, so maybe they're versions?
    fetch(`emojis/${pageNo}.json`)
        .then((response) => {
            if (!response.ok) {
                console.log(`Emoji load done, ${ytEmotes.size} emojis`);
                return null;
            }
        ...
        .then((list) => {
            if (list != null) {
                processYTEmojis(list);
                loadGlobalYTEmojiPage(pageNo + 1);
            }
        });
}

/**
 * This loads YouTube global emojis and any local emojis stored in custom_emojis.json
 * Sadly there's no official API to get emoji data for cutom emojis. There's some
 * reverse engineered methods, but they should probably be applied on the streamer.bot
 * side to work well.
 */
function loadYTEmojis() {
    const baseEmoteURL = 'https://yt3.ggpht.com/';
    const suffix = '=w24-h24-c-k-nd';

    // Add a bunch of hardcoded emotes that don't show up in definitions also
    const yt_emotes = {
    ...
    fetch('emojis/custom.json')
        .then((response) => {
            if (!response.ok) {
                console.log("No custom emojis loaded");
                return;
            }
            return response.json();
        })
...
async function pushMessage(type, message) {
    ...
    const msg = new Promise((resolve, reject) => {
        ...
        .then((badges) => {
            ...
            console.log(`Got badges, message type is ${message.type}`);
...
function removeMessagesbyUser(userId) {
    ...
    const messages = document.querySelectorAll(`.msg[data-user-id="${userId}"]`);
...
async function renderYTEmotes(message) {
    ...
    ytEmotes.forEach((emoteURL, emoteID, map) => {
